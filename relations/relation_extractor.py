...
class RelationExtractor:
    ...
    def relate_to_previous_entity(self, head_entity, ent_type, rel_type, context_size=100):
        relations = []
        prev = self.previous_entity_of_type(head_entity, ent_type, context_size=context_size)
        if prev == None:
            next = self.next_entity_of_type(head_entity, ent_type, context_size=context_size)
            if next != None:
    ...
    def relate_to_next_entity(self, head_entity, ent_type, rel_type, context_size=100):
        relations = []
        next = self.next_entity_of_type(head_entity, ent_type, context_size=context_size)
        if next == None:
            prev = self.previous_entity_of_type(head_entity, ent_type, context_size=context_size)
    ...
    def cpf_relations(self, entity):
        return self.relate_to_previous_entity(entity, "PESSOA", "cpf-pessoa")


    # Retorna relacoes (list[Relation]) que têm como "head" uma entidade do tipo CNPJ

    def cnpj_relations(self, entity):
        return self.relate_to_previous_entity(entity, "ORGANIZACAO", "cnpj-organizacao")
