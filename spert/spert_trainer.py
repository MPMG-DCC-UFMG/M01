...
class SpERTTrainer(BaseTrainer):
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader,
        ...
        # create evaluator
        predictions_path = os.path.join(self._log_path, 'predictions_{}_epoch_{}.json'.format(dataset.label, epoch))
        examples_path = os.path.join(self._log_path, 'examples_%s_{}_epoch_{}.html'.format(dataset.label, epoch))
